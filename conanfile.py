...
class GetTextConan(ConanFile):
    ...
    def source(self):
        source_data = self.conan_data["sources"]
        try:
           self.output.info("Trying to download using get()")
           get(self, **source_data[self.version], strip_root=True)
        except Exception as e:
           self.output.warning(f"get() failed with error: {e}")
           self.output.info("Falling back to ftp_download")
           ftp_download(self, **source_data['ftp'])
           archive_name = os.path.basename(source_data['ftp']["filename"])
           unzip(self, archive_name, strip_root=True)
